...
def check(ip_addr, stage, uri=None):
    """
    ...
    url = compose_url(ip_addr, uri)
    if stage == "initialClickLoginPage":
        return check_init_login()
    print(url)
    resp = requests.get(url, verify=False)
    soup = BeautifulSoup(resp.text.lower(),  "html.parser")
    logger.debug("got status=%d for %s\n" % (resp.status_code, resp.url))
    if resp.status_code == 301 or resp.status_code == 302:
        logger.debug("redirect should be automatically handled but not. source ip:%s, destiny:%s\n" % ip_addr, resp.url)
        ...
        return dev_info if check_login(compose_url(ip_addr, resp.url), resp, type_name) else False
        ...
        if type_name:
            ...
            if "loginUrlPattern" in dev_types[type_name]:
                ...
                return check_login(compose_url(ip_addr, _.group()), resp, type_name) if _ else False
            else:
                _ = dev_types[type_name]["nextUrl"]
                if _[0] == "string":
                    if not _[1]:
                        return check_login(compose_url(ip_addr), resp, type_name)
                    else:
                        return check_login(compose_url(ip_addr, _[1]), resp, type_name)
        elif stage == "look4LoginPage":
            pass
        elif not stage:
            meta_redirect_urls = soup.find('meta', attrs={'http-equiv': 'refresh'})['content'].partition('=')[2]
            return check(ip_addr, "look4LoginPage", meta_redirect_urls[0])
